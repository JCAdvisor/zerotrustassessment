...
Onboarding admin users with least privilege roles in Microsoft Defender for Office 365 (MDO) involves assigning only the minimum permissions necessary for admins to perform their tasks. This is done using role-based Controle de Acesso (RBAC) through Microsoft Entra ID and the Microsoft 365 Defender portal. Key roles include **Security Administrator**, **Security Reader**, and custom role groups tailored to specific responsibilities like incident response or policy management.
