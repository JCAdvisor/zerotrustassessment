...
**User Impact:** Medium  
Users may be prompted for multifactor authentication or blocked from accessing Cloud PCs based on risk signals, Dispositivos compliance, or location, which may require communication and support.

## Overview
Conditional Access for Windows 365 allows organizations to enforce Controle de Acessos based on user identity, Dispositivos compliance, location, and risk signals. These policies act as \"if-then\" rules---for example, if a user accesses a Cloud PC from an unmanaged Dispositivos, then they must complete multifactor authentication (MFA). Admins can target policies to the following Microsoft Entra apps:
...
Conditional Access policies are configured using Microsoft Entra ID (formerly Azure AD) and can be managed through Microsoft Intune. These policies help ensure that only trusted users and Dispositivos can access Cloud PCs, reducing the risk of unauthorized access. If not implemented, organizations risk exposing sensitive resources to compromised accounts or unmanaged Dispositivos.

This feature supports the Zero Trust principle of **\"Verify explicitly\"** by requiring authentication and authorization based on real-time signals such as user risk, Dispositivos health, and location.
