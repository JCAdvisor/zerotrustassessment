...
1. **Overview of Microsoft Tunnel**:
   - **Purpose**: Microsoft Tunnel allows secure access to on-premises resources from **iOS/iPadOS** and **Android Enterprise** Dispositivos using modern authentication and **Conditional Access**.
   ...
   - **Deployment Steps**:
     ...
     - Deploy the **Microsoft Defender for Endpoint** (the Tunnel client app) to Dispositivos.
     - Configure VPN profiles on iOS and Android Dispositivos to direct them to use the tunnel.
   - **Authentication Methods**:
     - Dispositivos use **Microsoft Entra ID** or **Active Directory Federation Services (AD FS)** to authenticate to the tunnel.
     - Conditional Access policies evaluate Dispositivos compliance before granting access.
...
2. **Benefits**:
   ...
   - **Per-App VPN**: Supports per-app VPN on iOS and Android Dispositivos.
...
4. **Impact to End Users**:
   - **Transparent**: For iOS Dispositivos with Microsoft Defender configured for per-app VPNs, users don't need to manually open or sign in to the app for the tunnel to work.
