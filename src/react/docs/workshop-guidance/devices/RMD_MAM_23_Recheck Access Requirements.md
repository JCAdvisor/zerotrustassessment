...
This setting is configured within existing Intune App Protection Policies and does not require infrastructure changes or Dispositivos enrollment.
...
This setting is available for both iOS/iPadOS and Android platforms and is part of the broader access requirements configuration. It ensures that even if a Dispositivos is left unattended or compromised, access to corporate data is not persistent without revalidation.

This feature supports the **Zero Trust principle of "Verify Explicitly"** by enforcing periodic reauthentication based on time, reducing the risk of unauthorized access due to session hijacking or unattended Dispositivos.
