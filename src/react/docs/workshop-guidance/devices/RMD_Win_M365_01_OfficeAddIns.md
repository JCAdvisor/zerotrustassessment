...
- Align well with Zero Trust principles:
  - **Verify explicitly**: Identity-based Controle de Acesso.
...
**Legacy Add-ins** refer to older, COM-based or VSTO (Visual Studio Tools for Office) extensions that integrate deeply with Office desktop applications. These are typically installed directly onto the Windows Dispositivos and can offer advanced integration capabilities not available to modern web-based add-ins. However, they also introduce greater security and compatibility risks due to their dependency on local system resources and elevated privileges.
