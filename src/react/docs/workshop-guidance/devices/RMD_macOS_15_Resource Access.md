...
**User Impact:** Low – Profiles are silently deployed to managed Dispositivos; no user interaction is typically required.

---

## Introduction

Resource access profiles in Intune allow administrators to configure secure access to corporate resources such as Wi-Fi networks, VPNs, and internal services using certificate-based authentication. For macOS, these profiles are essential for enforcing secure, password-less access and ensuring that only compliant, trusted Dispositivos can connect to sensitive infrastructure.

This section helps macOS administrators evaluate their current resource access configurations and align them with Zero Trust principles—particularly around Controle de Acesso, Dispositivos trust, and secure authentication.
...
- Supports Zero Trust by ensuring access is conditional on Dispositivos compliance and identity.
...
> This ensures that only trusted, compliant Dispositivos with valid certificates can connect to corporate networks, enforcing **explicit verification** and **Dispositivos trust** at the network edge.
...
- Use dynamic groups or filters to assign access profiles based on Dispositivos ownership, compliance state, or user role.
- Avoid assigning resource access profiles to unmanaged or non-compliant Dispositivos.

> From a Zero Trust perspective:  
> This supports **least privilege access** by ensuring that only the right users and Dispositivos receive access to sensitive network configurations.
...
> From a Zero Trust perspective:  
> Visibility into access provisioning is critical for **continuous trust**. Auditoria ensures that misconfigured or non-compliant Dispositivos are identified and remediated before they can access protected resources.

---

### BYOD Considerations

- For BYOD macOS Dispositivos, consider limiting access to internal resources unless certificate-based authentication is in place.
- Avoid deploying VPN or Wi-Fi profiles to personal Dispositivos unless explicitly required and secured.

> From a Zero Trust perspective:  
> BYOD Dispositivos are inherently less trusted. Limiting their access unless they meet strict authentication and compliance requirements supports **risk-based Controle de Acesso** and **assumes breach** by default.
...
- **Continuous trust**: Access is conditional on the Dispositivos maintaining compliance and holding valid certificates.  
...
- Assign access profiles only to compliant, corporate-managed macOS Dispositivos.  
- Use filters or dynamic groups to scope access based on Dispositivos role or ownership.  
- Monitor deployment status and validate certificate issuance before enabling access.  
- Limit access for BYOD Dispositivos unless strong authentication and compliance enforcement are in place.  
