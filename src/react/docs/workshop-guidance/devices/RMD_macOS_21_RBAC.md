# Role-Based Controle de Acesso (RBAC)
...
Role-Based Controle de Acesso (RBAC) in Intune allows organizations to delegate administrative permissions based on roles, responsibilities, and scope. For macOS environments, RBAC ensures that only authorized personnel can manage Dispositivos, deploy configurations, or access sensitive data. This is a foundational control in a Zero Trust model, where **access is granted based on least privilege and verified identity**.
...
- **Improves operational security** by limiting who can manage macOS Dispositivos and policies.  
...
These roles can be assigned to users or groups and scoped to specific Dispositivos groups.
...
> From a Zero Trust perspective: Custom roles enable **fine-grained Controle de Acesso** and **role separation**.
...
- Assign roles to macOS-specific IT teams to manage only macOS Dispositivos and policies.  
...
- **Defense in depth**: RBAC complements Conditional Access, compliance policies, and Dispositivos restrictions.  
...
- [Role-Based Controle de Acesso in Intune](https://learn.microsoft.com/en-us/mem/intune/fundamentals/role-based-access-control)  
