...
## Overview
In **Microsoft Intune**, you can enhance your Auditoria and tracking capabilities by sending logs to **Azure Monitor** using **Diagnostics Settings**. Here's how it works:
...
1. **Audit Logs**: These logs record activities that generate changes in Intune, such as creating, updating, deleting, assigning, and remote actions. You can route these logs to:
   ...
   - **Log Analytics**: Enable rich visualizations, Auditoria, and alerting¹.

2. **Operational Logs**: These provide details on user and Dispositivos enrollment (success or failure) and noncompliant Dispositivos. Similar to audit logs, you can route them to the same services¹.
