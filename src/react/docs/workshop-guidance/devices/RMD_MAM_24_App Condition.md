...
This setting is part of standard app protection policy configuration and can be applied without infrastructure changes or Dispositivos enrollment.
...
This mechanism ensures that only compliant apps are allowed to access sensitive data, even on unmanaged Dispositivos. For example, if a user attempts to access corporate data using an outdated or unapproved app, the policy can block access or prompt the user to update the app.
