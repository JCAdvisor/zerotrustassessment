...
### Benefits
1. **Enhanced Security**: Windows LAPS ensures that each Dispositivos has a unique, complex local administrator password, reducing the risk of unauthorized access.
...
1. **Enable Windows LAPS**: In the Azure Active Directory (AAD) or Microsoft Entra Admin portal, navigate to Dispositivos > All Dispositivos > Dispositivos Settings. Toggle the option to enable Microsoft Entra Local Administrator Password Solution (LAPS) to "Yes" and save.
...
3. **Assign Policies**: Assign the LAPS policy to the appropriate groups of Dispositivos or users.
