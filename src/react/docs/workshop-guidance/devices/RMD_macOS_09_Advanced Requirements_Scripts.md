...
**Implementation Effort:** Medium – IT admins need to write, test, and deploy shell scripts, and ensure they are scoped correctly to Dispositivos groups and executed securely.  
...
Scripts are a powerful tool for macOS administrators using Intune. They allow IT teams to automate configuration, enforce settings not natively exposed in Intune, and remediate issues at scale. Scripts are especially valuable in macOS environments where MDM capabilities may be limited compared to other platforms. From a Zero Trust perspective, scripts help enforce **Dispositivos integrity**, **configuration consistency**, and **least privilege** by automating secure baselines.

## Why This Matters

- **Extends Intune’s native capabilities** for macOS management.
- **Automates secure configuration** and remediation tasks.
- **Supports Zero Trust** by enforcing consistent, policy-driven Dispositivos states.
- **Reduces manual intervention** and human error.
- **Improves visibility and control** over Dispositivos posture.

## Key Considerations

### Supported Script Types

- Intune supports **shell scripts** for macOS Dispositivos.
- Scripts can be written in `sh`, `bash`, or `zsh`, as long as the appropriate shebang (e.g., `#!/bin/bash`, `#!/usr/bin/env zsh`) is included and the shell is available on the Dispositivos.
- **Scripts are executed by the Intune Management Agent**, which is **automatically installed** on macOS Dispositivos during Intune enrollment.
...
From a Zero Trust perspective: These use cases support **least privilege**, **Dispositivos hardening**, and **continuous trust enforcement**.

### Assignment and Targeting

- Scripts can be assigned to **Dispositivos groups**.
...
- Scripts run as **root** by default and execute once per Dispositivos unless configured otherwise.
...
### Auditoria and Logging

- Script output is available in the **Intune admin center** for each Dispositivos.
...
- **Verify explicitly**: Scripts enforce Dispositivos configurations that can be validated and monitored.
