...
**Implementation Effort:** Low – IT administrators only need to configure a setting within the App Protection Policy or Dispositivos restriction policy, depending on the platform.
...
- **Android**: Select Block to block screen capture, block Circle to Search, and block Google Assistant accessing org data on the Dispositivos when using this app. Choosing Block will also blur the App-switcher preview image when using this app with a work or school account.
