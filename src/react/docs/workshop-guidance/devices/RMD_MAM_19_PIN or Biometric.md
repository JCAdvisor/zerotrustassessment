...
This is a straightforward configuration within Intune App Protection Policies and does not require Dispositivos enrollment or infrastructure changes.
...
Microsoft Intune App Protection Policies (APP) allow organizations to enforce access requirements such as PIN or biometric authentication (e.g., Face ID, fingerprint) before users can access corporate data within managed apps. These settings help ensure that only authorized users can access sensitive information, even on personal or unmanaged Dispositivos. Admins can configure whether a PIN is required, whether biometric methods are allowed, and how frequently reauthentication is needed.

This feature supports the **Zero Trust principle of "Verify Explicitly"** by requiring strong user authentication based on identity and Dispositivos context. Without this control, unauthorized users could potentially access corporate data if a Dispositivos is lost, stolen, or shared.
