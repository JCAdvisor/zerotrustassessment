...
1. **Prerequisites**:
   - Ensure Dispositivos are running macOS 13.0 or newer.
...
3. **Create Platform SSO Policy in Intune**:
   ...
   - Select **macOS** for the platform and **Dispositivos features** for the profile type.
   - Configure the SSO app extension settings¹.

4. **Deploy the Company Portal App**:
   - Ensure the Company Portal app is deployed to all macOS Dispositivos.

5. **Enroll Dispositivos and Apply Policies**:
   - Enroll the macOS Dispositivos in Intune.
   - Assign the SSO policy to the relevant user groups.

6. **Confirm Settings on Dispositivos**:
   - Verify that the SSO settings are correctly applied on the Dispositivos.
...
- **Conditional Access**: Ensures that only compliant Dispositivos can access corporate resources.
...
- **Conditional Access**: Enforces policies that require Dispositivos to meet security standards before granting access.
- **Least Privilege Access**: Limits access to resources based on user roles and compliance status.

## Reference

* Configure Platform SSO for macOS Dispositivos | Microsoft Learn. https://learn.microsoft.com/en-us/mem/intune/configuration/platform-sso-macos.
* Use the Microsoft Enterprise SSO plug-in on macOS Dispositivos. https://learn.microsoft.com/en-us/mem/intune/configuration/use-enterprise-sso-plug-in-macos-with-intune.
* Microsoft Enterprise SSO plug-in for Apple Dispositivos. https://learn.microsoft.com/en-us/entra/identity-platform/apple-sso-plugin.
