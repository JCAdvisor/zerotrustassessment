...
**Implementation Effort:** Medium – Admins must configure and assign enrollment profiles and optionally customize account settings during Automated Dispositivos Enrollment (ADE).  
...
Managing local accounts on macOS Dispositivos is a critical part of securing the endpoint and enforcing consistent identity and Controle de Acessos. In Intune, administrators can configure how local accounts are created, named, and managed during Automated Dispositivos Enrollment (ADE). This section helps macOS administrators evaluate their local account strategy to ensure it aligns with Zero Trust principles—particularly around identity assurance, privilege minimization, and lifecycle control.

This guidance applies to corporate-owned macOS Dispositivos enrolled via ADE, and in some cases, to BYOD Dispositivos where script-based controls are applicable.
...
- **Standardizes account naming and creation** across Dispositivos.  
- **Supports Zero Trust** by ensuring consistent identity and Controle de Acessos.  
...
- Enforce password complexity and rotation policies using **Dispositivos configuration profiles**.  
...
- Regularly audit local accounts on managed Dispositivos to ensure they align with policy.  

### BYOD Considerations

- While Intune does not control account creation on BYOD Dispositivos, **scripts can still be deployed** to demote local admin accounts to standard—provided the Dispositivos is enrolled and script execution is permitted.  
...
- **Minimized Acesso Privilegiado**: Using scripts to manage account roles reduces the need for manual IT intervention and enforces policy at scale.  
...
- **Use scripts to demote admin accounts** post-enrollment, including on BYOD Dispositivos where applicable.  
...
- [Configure macOS Enrollment Profiles in Intune](https://learn.microsoft.com/en-us/mem/intune/enrollment/Dispositivos-enrollment-program-enrollment-profile)  
- [macOS Dispositivos Configuration Settings in Intune](https://learn.microsoft.com/en-us/mem/intune/configuration/Dispositivos-profile-create)  
