...
## Overview
Mobile Threat Defense (MTD) integration with Microsoft Intune allows you to enhance security for mobile Dispositivos. Here's how it works:

1. **Purpose**:
   - Intune integrates data from third-party MTD vendors as an information source for Dispositivos compliance policies and Conditional Access rules.
   - This integration helps protect corporate resources (such as Exchange and SharePoint) by blocking access from compromised mobile Dispositivos.

2. **How It Works**:
   - Intune uses a Mobile Threat Defense connector to communicate with your chosen MTD vendor.
   - MTD partners offer easy-to-deploy applications for mobile Dispositivos that actively scan and analyze threat information.
   ...
   - Based on the comparison, access to specific resources can be revoked if the Dispositivos is compromised.
