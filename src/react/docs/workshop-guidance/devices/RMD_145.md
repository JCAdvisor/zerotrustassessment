...
## Overview
### Deploying Office Apps on Windows Dispositivos in Microsoft Intune

**Deploying Office apps** on Windows Dispositivos using Microsoft Intune involves assigning and installing Microsoft 365 apps (formerly Office 365) to managed Dispositivos. This process ensures that users have access to essential productivity tools while maintaining organizational control over software deployment.
...
3. **Consistency**: Ensures a consistent set of applications across all managed Dispositivos, reducing compatibility issues.
...
- **Ensuring Compliance**: Only compliant Dispositivos can install and use Office apps, enforcing security policies.
- **Continuous Auditoria**: Dispositivos are continuously monitored to ensure they remain compliant with security standards.
- **Conditional Access**: Integration with Conditional Access policies ensures that only trusted Dispositivos and users can access Office apps.

## Reference

*  Add Microsoft 365 Apps to Windows 10/11 Dispositivos with Microsoft Intune. https://learn.microsoft.com/en-us/mem/intune/apps/apps-add-office365
