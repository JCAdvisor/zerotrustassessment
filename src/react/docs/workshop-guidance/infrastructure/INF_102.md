...
- **Block access to unscanned or malicious files** using Microsoft Entra Attribute-Based Controle de Acesso (ABAC). This ensures only clean files are accessible to users and applications.
...
- **Move malicious files to quarantine** by transferring them to a dedicated container or storage account with restricted access. Use Microsoft Entra ID and role-based Controle de Acesso (RBAC) to limit access to security personnel only.
