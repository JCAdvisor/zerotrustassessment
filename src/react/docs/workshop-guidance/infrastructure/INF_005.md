...
Assigning roles in Microsoft Defender for Cloud is essential for enforcing Controle de Acesso and ensuring that users, groups, and services have the appropriate permissions to perform their tasks. Defender for Cloud uses Azure Role-Based Controle de Acesso (RBAC) to manage access at the subscription, resource group, or resource level. Built-in roles such as **Owner**, **Contributor**, and **Reader** are supplemented by Defender-specific roles like **Security Reader** and **Security Admin**.
...
- **Contributor/Owner**: Can take full action on resources, including enabling Defender plans and configuring Auditoria components.
