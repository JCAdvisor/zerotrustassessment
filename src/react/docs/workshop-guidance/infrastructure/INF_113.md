...
**Implementation Effort:** Medium — Requires configuration of automated workflows, Controle de Acessos, and integration with security operations tools.
...
1. **Block Access to Malicious or Unscanned Files**  
   Use Microsoft Entra Attribute-Based Controle de Acesso (ABAC) to restrict access to blobs based on malware scan results. This ensures only clean files are accessible to users and applications [1](https://learn.microsoft.com/en-us/azure/defender-for-cloud/defender-for-storage-configure-malware-scan).
