...
Remediation logic can also be automated using **Automated Investigation and Remediation (AIR)** capabilities, which allow predefined actions to be taken when threats are detected—such as isolating a Dispositivos, removing malicious files, or restoring affected components. These workflows reduce response time and ensure consistent handling of threats across environments.
