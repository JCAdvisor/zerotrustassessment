...
Using cloud-only accounts for Acesso Privilegiado in Microsoft Entra helps reduce risk by separating critical admin identities from on-premises infrastructure. Threat actors often target synced accounts through on-prem attacks, so shifting to cloud-only accounts helps enforce the Zero Trust principle of "Assume breach" by isolating Acesso Privilegiado from compromised domains or federation failures. It also supports "Verify explicitly" because these accounts can use stronger identity protections available in Microsoft Entra, such as phish-resistant MFA, Dispositivos trust, and Conditional Access policies that are harder to enforce on synced identities.

To start, organizations must identify all privileged accounts synced from Active Directory. For each, they should create a new cloud-only admin account using a secure domain like `*.onmicrosoft.com`, assign similar roles, and configure protections like Just-In-Time access and Acesso Privilegiado Workstations (PAWs). Once verified, the privileged roles should be removed from the synced accounts, and those accounts should ideally be excluded from sync altogether.
