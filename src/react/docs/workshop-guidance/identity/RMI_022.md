...
Use Entra ID B2B collaboration for new external Identities (business partners and guests). B2B accounts offer several benefits over local guest accounts for managing Colaboração Externas:
...
* Streamline Security Controls: Cross-tenant access settings allow you to configure whether to trust the MFA or Dispositivos claims for external organizations that are also in Entra ID. In the context of zero trust, the Dispositivos health claim from external organizations is an important consideration.
