...
**Implementation Effort:** High – Identity governance teams must define access requirements, configure access packages, and establish approval workflows and approvers tailored to Colaboração Externa scenarios.

**User Impact:** High – Guest users will need guidance to a self-service onboarding process.

## Overview

Configuring access packages for guest users in Microsoft Entra ID involves creating structured and policy-driven access pathways that align with the organization's collaboration needs. This process begins by identifying common Colaboração Externa scenarios and determining the specific resources—such as Microsoft Teams, SharePoint sites, or enterprise applications—that external users require access to based on their business roles, and model those as catalogs, connected organizations, and approvals. 
