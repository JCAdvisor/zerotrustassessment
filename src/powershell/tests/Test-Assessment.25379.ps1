...
function Test-Assessment-25379 {
    [ZtTest(
    	...
    	TestId = 25379,
        Title = 'As políticas de Conditional Access usam controles de rede compatível',
        UserImpact = 'Médio'
    )]
    [CmdletBinding()]
    param()
    ...
    $activity = 'Verificando controles de rede compatível em Conditional Access'
    Write-ZtProgress -Activity $activity -Status 'Recuperando o status de sinalização do Global Secure Access'

    # Q1: Retrieve Global Secure Access Conditional Access signaling status
        # Consulta the Global Secure Access settings to determine if Conditional Access signaling is enabled
    try {
        $settings = Invoke-ZtGraphRequest -RelativeUri 'networkAccess/settings/conditionalAccess' -ApiVersion beta -ErrorAction Stop
    }
    catch {
        Write-PSFMessage -Message "Falha ao recuperar as configurações de Conditional Access do Global Secure Access via Graph: $_" -Tag Test, Graph -Level Warning
        $settings = $null
    }

    # Q2: Retrieve the compliant network named location
        # Consulta named locations to find the compliant network named location
    Write-ZtProgress -Activity $activity -Status 'Recuperando local nomeado de rede compatível'
    ...
    # Q3: Retrieve enabled Conditional Access policies that use compliant network location
        # Consulta all enabled Conditional Access policies to find those that reference the compliant network location
    Write-ZtProgress -Activity $activity -Status 'Recuperando políticas habilitadas de Conditional Access'
    ...
    if (-not $settings -or $settings.signalingStatus -ne 'enabled') {
        ...
        $testResultMarkdown = if (-not $settings) {
            "❌ **Falha**: Não foi possível recuperar o status de sinalização do Global Secure Access.`n`n%TestResult%"
        } else {
            "❌ **Falha**: A sinalização do Global Secure Access está desabilitada. Os controles de rede compatível não podem funcionar sem este pré-requisito.`n`n%TestResult%"
        }
    }
    elseif (-not $compliantNetworkLocation) {
        $passed = $false
        $customStatus = $null
        $testResultMarkdown = "❌ **Falha**: O local nomeado de rede compatível não existe ou não está configurado corretamente.`n`n%TestResult%"
    }
    elseif (-not $policies) {
        $passed = $false
        $customStatus = $null
        $testResultMarkdown = "❌ **Falha**: Nenhuma política habilitada de Conditional Access encontrada no locatário.`n`n%TestResult%"
    }
    else {
        ...
        if ($standardPolicies) {
            $passed = $true
            $customStatus = $null
            $testResultMarkdown = "✅ **Sucesso**: Os controles de rede compatível estão configurados no Conditional Access. A sinalização do Global Secure Access está habilitada e pelo menos uma política de Conditional Access bloqueia o acesso de todas as localizações, exceto a rede compatível.`n`n%TestResult%"
        }
        elseif ($alternativePolicies) {
            ...
            $passed = $false
            $customStatus = $null
            $testResultMarkdown = "❌ **Falha**: Os controles de rede compatível não estão configurados corretamente. Nenhuma política de Conditional Access referência o local de rede compatível.`n`n%TestResult%"
        }
    }
...
### [Políticas de Conditional Access usando rede compatível]($caPoliciesLink)

| Status | Nome da política | Estado | Localizações incluídas | Localizações excluídas | Controles de concessão |
| :----- | :---------------- | :---- | :-------------------- | :-------------------- | :-------------------- |
$policyTableRows
"@
    }
...
### [Políticas de Conditional Access]($caPoliciesLink)

Foram encontradas $($policies.Count) políticas habilitadas de Conditional Access, mas não é possível avaliar o uso da rede compatível sem um local de rede compatível válido.
"@
    }
    elseif (-not $policies) {
        $policiesSection = @"
...
❌ Nenhuma política habilitada de Conditional Access foi encontrada no locatário.
"@
    }
    else {
        $policiesSection = @"
...
❌ Nenhuma política habilitada de Conditional Access referencia o local de rede compatível.
"@
    }
