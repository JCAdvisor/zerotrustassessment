Protocolos de autenticação legada, como autenticação básica para SMTP e IMAP, não suportam recursos modernos de segurança como a autenticação multifator (MFA). Isso torna as contas vulneráveis a ataques de força bruta e pulverização de senhas (password spray).
