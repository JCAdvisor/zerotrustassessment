...
function Test-Assessment-25383 {
    [ZtTest(
    	...
    	TestId = 25383,
        Title = 'Os privilégios administrativos são rigidamente limitados para evitar comprometimento',
        UserImpact = 'Baixo'
    )]
    [CmdletBinding()]
    param(
    ...
    $activity = 'Verificando atribuições de função de Global Administrator e GSA'
    Write-ZtProgress -Activity $activity -Status 'Obtendo definições de função'

        # Consulta Q1: Get role definitions for GA and GSA from database
    $sqlRoleDefinitions = @"
...
"@
    ...
    $tenantId = (Get-MgContext).TenantId
        ...
        # Consulta Q2/Q3: Get role assignments for GA and GSA roles from vwRole view with user details
...
"@
    ...
    foreach ($roleInfo in $rolesToCheck) {
        ...
        foreach ($assignment in $assignments) {
            ...
            if ($principalType -eq 'group') {
                $assignmentInfo.Issue = 'Atribuição de grupo'
                $result.Issues += $assignmentInfo
            }
            elseif ($principalType -eq 'servicePrincipal') {
                $assignmentInfo.Issue = 'Atribuição de service principal'
                $result.Issues += $assignmentInfo
            }
            elseif ($principalType -eq 'user' -and $assignment.userType -eq 'Guest') {
                $assignmentInfo.Issue = 'Atribuição de usuário convidado'
                $result.Issues += $assignmentInfo
            }
            elseif ($principalType -eq 'user' -and $assignment.accountEnabled -eq $false) {
                ...
                $assignmentInfo.Issue = 'Conta desabilitada com função privilegiada'
    ...
    if ($gaNotFound) {
        $testResultMarkdown = "❌ A definição de função Global Administrator não foi encontrada. Essa é uma função crítica que deve existir em todos os locatários.`n`n%TestResult%"
    }
    # Fail: Groups, guests, or service principals assigned
    elseif ($hasAnyIssues) {
        $testResultMarkdown = "❌ As funções GA/GSA incluem grupos, convidados ou service principals que exigem revisão imediata.`n`n%TestResult%"
    }
    # Investigate: Disabled users with privileged roles - latent risk
    elseif ($hasAnyDisabledUsers) {
        $passed = $true
        $customStatus = 'Investigate'
        $testResultMarkdown = "⚠️ As funções GA/GSA incluem usuários Membro desabilitados ou excedem os limites de atribuição recomendados; revise e remedeie.`n`n%TestResult%"
    }
    ...
    # Pass: All principals are enabled Member users within limits
        ...
        $passed = $true
        $testResultMarkdown = "✅ As funções GA/GSA são limitadas a usuários Membro habilitados e avaliados; nenhum grupo, convidado, service principal ou conta desabilitada está atribuído, e as contagens de atribuição estão dentro dos limites aprovados.`n`n%TestResult%"
    }
    ...
    foreach ($result in $allResults) {
        $mdInfo += "`n## [$($result.RoleName) — Atribuições](https://entra.microsoft.com/#view/Microsoft_AAD_IAM/RolesManagementMenuBlade/~/AllRoles)`n`n"

        if (-not $result.Found) {
            if ($result.RoleName -eq 'Global Administrator') {
                $mdInfo += "❌ A definição da função $($result.RoleName) não foi encontrada. Esta é uma função crítica que deve existir.`n`n"
            }
            else {
                $mdInfo += "ℹ️ A definição da função $($result.RoleName) não foi encontrada. Pulando a avaliação (a função pode não estar provisionada no locatário).`n`n"
            }
            continue
        }

        $mdInfo += "**ID da definição de função**: $($result.RoleDefinitionId)  `n"
        $mdInfo += "**Contagem total de atribuições**: $($result.TotalCount)  `n"
        $mdInfo += "**Contagem de atribuições válidas**: $($result.ValidAssignments.Count)  `n"
        $mdInfo += "**Contagem de problemas**: $($result.Issues.Count + $result.DisabledAssignments.Count)  `n`n"

        if ($result.Issues.Count -gt 0) {
            $mdInfo += "### ❌ Atribuições não conformes`n`n"
            $mdInfo += "| Nome | Nome principal | Tipo | Tipo de usuário | Conta habilitada | Status |`n"
            $mdInfo += "| :----------- | :-- | :--- | :-------- | :-------------- | :----- |`n"
            ...
            foreach ($issue in $displayIssues) {
                $mdInfo += "| $(Get-SafeMarkdown $issue.DisplayName) | $(Get-SafeMarkdown $issue.UPN) | $($issue.Type) | $($issue.UserType) | $($issue.AccountEnabled) | Falha |`n"
            }
            ...
            if ($result.Issues.Count -gt $maxDisplay) {
                ...
                $mdInfo += "`n*Exibindo os primeiros $maxDisplay de $($result.Issues.Count) registros. [Visualizar todas as atribuições no portal Entra]($portalUrl)*`n"
            }
            $mdInfo += "`n"
        }

        if ($result.DisabledAssignments.Count -gt 0) {
            $mdInfo += "### ⚠️ Contas desabilitadas com funções privilegiadas`n`n"
            $mdInfo += "| Nome | Nome principal | Tipo | Tipo de usuário | Conta habilitada | Status |`n"
            $mdInfo += "| :----------- | :-- | :--- | :-------- | :-------------- | :----- |`n"
            ...
            foreach ($disabled in $displayDisabled) {
                $mdInfo += "| $(Get-SafeMarkdown $disabled.DisplayName) | $(Get-SafeMarkdown $disabled.UPN) | $($disabled.Type) | $($disabled.UserType) | $($disabled.AccountEnabled) | Investigar |`n"
            }

            if ($result.DisabledAssignments.Count -gt $maxDisplay) {
                $roleNameEncoded = [System.Uri]::EscapeDataString($result.RoleName)
                $portalUrl = "https://entra.microsoft.com/#view/Microsoft_Azure_PIMCommon/UserRolesViewModelMenuBlade/~/members/roleObjectId/$($result.TemplateId)/roleId/$($result.TemplateId)/roleTemplateId/$($result.TemplateId)/roleName/$roleNameEncoded/isRoleCustom~/false/resourceScopeId/%2F/resourceId/$tenantId"
                $mdInfo += "`n*Exibindo os primeiros $maxDisplay de $($result.DisabledAssignments.Count) registros. [Visualizar todas as atribuições no portal Entra]($portalUrl)*`n"
            }
            $mdInfo += "`n"
        }

        if ($result.ValidAssignments.Count -gt 0) {
            $mdInfo += "### ✅ Atribuições válidas de usuário Membro`n`n"
            $mdInfo += "| Nome | Nome principal | Tipo | Tipo de usuário | Conta habilitada | Status |`n"
            $mdInfo += "| :----------- | :-- | :--- | :-------- | :-------------- | :----- |`n"
            ...
            foreach ($valid in $displayValid) {
                $mdInfo += "| $(Get-SafeMarkdown $valid.DisplayName) | $(Get-SafeMarkdown $valid.UPN) | $($valid.Type) | $($valid.UserType) | $($valid.AccountEnabled) | Válido |`n"
            }

            if ($result.ValidAssignments.Count -gt $maxDisplay) {
                $roleNameEncoded = [System.Uri]::EscapeDataString($result.RoleName)
                $portalUrl = "https://entra.microsoft.com/#view/Microsoft_Azure_PIMCommon/UserRolesViewModelMenuBlade/~/members/roleObjectId/$($result.TemplateId)/roleId/$($result.TemplateId)/roleTemplateId/$($result.TemplateId)/roleName/$roleNameEncoded/isRoleCustom~/false/resourceScopeId/%2F/resourceId/$tenantId"
                $mdInfo += "`n*Exibindo os primeiros $maxDisplay de $($result.ValidAssignments.Count) registros. [Visualizar todas as atribuições no portal Entra]($portalUrl)*`n"
            }
            $mdInfo += "`n"
        }
    }
    ...
    $params = @{
        TestId = '25383'
        Title  = 'Privilégios de administrador Global e GSA são rigidamente limitados para evitar comprometimento do locatário'
        Status = $passed
