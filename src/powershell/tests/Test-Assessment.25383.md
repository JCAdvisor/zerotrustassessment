...
Over-privileged assignments also increase blast radius when credentials are phished or session tokens are stolen. Limiting these roles to a small, vetted set of administrators—and Auditoria assignments for groups, guests, and service principals—reduces the tenant-wide attack surface and enforces least privilege.

**Remediation action**

Review and limit GA/GSA assignments:
- [Role-based Controle de Acesso permissions reference](https://learn.microsoft.com/entra/identity/role-based-access-control/permissions-reference)
