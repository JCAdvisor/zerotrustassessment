...
.DESCRIPTION
    ...
    Conditional Access policy with meaningful security controls (MFA, Dispositivos compliance, or authentication strength).
                    ...
                    if ($null -ne $policy.grantControls) {
                        ...
                        if ($null -ne $policy.grantControls.builtInControls -and $policy.grantControls.builtInControls.Count -gt 0) {
                            foreach ($control in $policy.grantControls.builtInControls) {
                                if ($control -in @('mfa', 'compliantDispositivos', 'domainJoinedDispositivos', 'approvedApplication')) {
