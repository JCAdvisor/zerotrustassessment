...
export function DataTable<TData extends Test, TValue>({
    ...
    const table = useReactTable({
        data: isOverview ? data : filteredData,
        columns,
    ...
    const overviewData = filteredData.reduce((result: { [ key: string ]: Test[] }, test: Test) => {
        const newCategory: string | null = test["TestCategory"];
    ...
    return (
        ...
        <div>
                ...
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="text-xs text-muted-foreground">
                            Mostrando {filteredData.length} de {pillarFilteredData.length} testes
                        </div>
                    </div>
                </div>
            ...
            <Table>
                ...
                <TableBody>
                    {
                        Object.entries(overviewData).map(([category, tests]: [string, Test[]]) => {
                        const failedCount = tests.filter(
                            test => test.TestStatus == "Failed"
                        ).length
                    ...
                    <TableBody>
                        {table.getRowModel().rows?.length ? (
                            table.getRowModel().rows.map((row) => (
                                <TableRow
                                    className={`${isOverview ? "" : "cursor-pointer" }`}
                                    key={row.id}
                            ...
                            <CardHeader>
                                ...
                                <div className="mt-2 flex w-full justify-between text-sm">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-semibold">Impacto para usuário:</span>
                    ...
                    <div className="grid pt-10 gap-6">
                        <Card>
                            <CardHeader><CardTitle>
                                <div className="flex">
                                    <span className="pr-3"> Resultados do teste → </span><StatusIcon Item={selectedRow!} />
                                </div></CardTitle>
                            </CardHeader>
                            <CardContent>
